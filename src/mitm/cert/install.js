...
const ROOT_CA_CN = "Grouter MITM Root CA";
...
async function installCertMac(sudoPassword, certPath) {
  ...
  const deleteOld = `security delete-certificate -c "Grouter MITM Root CA" /Library/Keychains/System.keychain 2>/dev/null || true`;
...
async function installCertLinux(sudoPassword, certPath) {
  ...
  const config = getLinuxCertConfig();
  const destFile = `${config.dir}/grouter-root-ca.crt`;
