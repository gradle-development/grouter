...
## Added
- **Upstream sync (post-0.9.1)** — antigravity executor + tool decloaking; `cloudCodeThinking` (OmniRoute parity), model-aware `thinkingConfig` strip, hidden thinking tokens in stream; OpenAI→Claude reverse-translate on `/v1/messages` for non-Claude providers; GCP `projectId` override + API-loaded project dropdown (gemini-cli/antigravity); shared `connectionsPagination` + tests
- **Standalone / MITM build** — copy entire `src/` into standalone; ESM `package.json` under `src/`; `mitmToolHosts` renamed to `.cjs`
- **Provider icons** — official WebP logos for many providers; alias maps (Grok CLI→Grok, ClinePass→Cline, MiMo Free→MiMo, Perplexity Agent→Perplexity, muse-spark-web→`muse.webp`)
- **Sidebar** — “check new changelog” link on update notification
- **npm trusted publishing** — OIDC publish flow; pin latest npm; `repository` metadata for provenance

## Fixed
- **Grok CLI reactivation** — more reliable reactivation path for Grok CLI accounts
- **Docker SQLite volume** — keep volume name `9router-data`; rename creates empty DB with no error
- **Custom endpoint API key form** — drop redundant `defaultModel` field for OpenAI/Anthropic-compatible custom endpoints
- **CLI tools assets** — `.webp` icons (404 fix)
- **gemini-cli** — strip `reasoning_effort`; surface `apiKey` in request details
- **Provider detail** — pagination restored; select-all + URL page sync + autoclaw icon cache-bust
- **Version check** — npm package name check uses VansRouter where required for publish/version tooling

## Changed
- Root and CLI package versions bumped to **0.9.4**.
