---
name: grouter-tts
description: Text-to-speech via Grouter /v1/audio/speech using OpenAI / ElevenLabs / Deepgram / Edge TTS / Google TTS / Hyperbolic / Inworld voices. Use when the user wants to convert text to speech, generate audio, voiceover, narrate, or read text aloud.
---

# Grouter — Text-to-Speech

Requires `GROUTER_URL` (and `GROUTER_KEY` if auth enabled). See https://raw.githubusercontent.com/decolua/grouter/refs/heads/master/skills/grouter/SKILL.md for setup.

## Discover

```bash
# 1) List models
curl $GROUTER_URL/v1/models/tts | jq '.data[].id'
# 2) Per-model metadata (params, voicesUrl if voice-by-id)
curl "$GROUTER_URL/v1/models/info?id=el/eleven_multilingual_v2"
# 3) List voices (elevenlabs, edge-tts, deepgram, inworld, local-device). Optional ?lang=vi
curl "$GROUTER_URL/v1/audio/voices?provider=edge-tts&lang=vi" | jq '.data[].model'
```
...
`POST $GROUTER_URL/v1/audio/speech`
...
```bash
curl -X POST "$GROUTER_URL/v1/audio/speech" \
  -H "Authorization: Bearer $GROUTER_KEY" \
...
const r = await fetch(`${process.env.GROUTER_URL}/v1/audio/speech`, {
  method: "POST",
  headers: { "Authorization": `Bearer ${process.env.GROUTER_KEY}`, "Content-Type": "application/json" },
