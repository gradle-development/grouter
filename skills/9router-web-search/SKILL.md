---
name: grouter-web-search
description: Web search via Grouter /v1/search using Tavily / Exa / Brave / Serper / SearXNG / Google PSE / Linkup / SearchAPI / You.com / Perplexity. Use when the user wants to search the web, look up information, find articles, or query a search engine.
---
...
```bash
curl $GROUTER_URL/v1/models/web | jq '.data[] | select(.kind=="webSearch") | .id'
# Per-provider params (searchTypes, maxResults, required options like cx for google-pse)
curl "$GROUTER_URL/v1/models/info?id=tavily/search"
```
...
```bash
curl -X POST $GROUTER_URL/v1/search \
  -H "Authorization: Bearer $GROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"tavily","query":"Grouter open source","max_results":5}'
```

JS:

```js
const r = await fetch(`${process.env.GROUTER_URL}/v1/search`, {
  method: "POST",
