...
function cleanupLegacySystray({ silent = false } = {}) {
  ...
  // 2) npm global nested: <npm_prefix>/node_modules/grouter/node_modules/systray
  ...
  for (const dir of targets) {
    if (fs.existsSync(dir)) {
      try {
        ...
        if (!silent) console.log(`[grouter][runtime] removed legacy systray: ${dir}`);
      } catch (e) {
        if (!silent) console.warn(`[grouter][runtime] failed to remove ${dir}: ${e.message}`);
      }
    }
  }
}
...
function chmodSystrayBin({ silent = false } = {}) {
  ...
  try {
    ...
    if (!silent) console.warn(`[grouter][runtime] chmod tray bin failed: ${e.message}`);
  }
}
